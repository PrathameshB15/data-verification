...
def get_crm_credentials(client_id=None):
    """Fetch VRIO CRM credentials from the database. If client_id is None, fetch all active clients."""
    try:
...
def main():
    ...
    parser.add_argument(
        "--client_id", type=str, help="Client ID for verification (default: all active VRIO clients)"
    )
    parser.add_argument(
        "--date",
        type=str,
    ...
    if not crm_list:
        print(f"No VRIO CRM credentials found" + (f" for client_id: {args.client_id}" if args.client_id else ""))
        return
    ...
    filename = f"vrio_verification_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
